...
import { ArrowUp, ToolCaseIcon, X, Square, Mic, CheckCircle2, Terminal, Cpu, ChevronDown, Dot, RefreshCw, XCircle, Box } from "lucide-react";
...
import { Googledocstool } from "@/features/toolsselection";
...
export const Googledocs = () => {
    ...
    useEffect(() => {
        const fetchchatmessage = async () => {
            try {
                ...
                const response = await fetchdocsmessage();
                if (response.success) {
    ...
    const adddocsurl = async () => {
        try {
            const response = await addgoogledocs(docsinput);
            if (response.success) {
    ...
    const selecteddocsTitle = useMemo(() => {
        return docs.find(g => g.url === docsurl)?.name || "";
    }, [docsurl, docs]);
    ...
    const docsmsgdelete = async () => {
        try {
            const response = await deletedocsmsg();
            if (response.success) {
                toast.success(response.message);
                setrefresh(prev => !prev);
            }
            ...
            <div className="flex h-[92vh] w-full flex-col bg-background">
                <div className="mx-auto w-full max-w-5xl flex justify-between gap-1">
                    <div className="flex flex-col gap-1">
                        <h1 className="text-2xl font-bold flex items-center gap-3"><img src="https://upload.wikimedia.org/wikipedia/commons/0/01/Google_Docs_logo_%282014-2020%29.svg" className="w-7 h-7" /> GoogleDocsAgent</h1>
                ...
                <div className="flex w-full gap-2 justify-between mx-auto max-w-5xl mb-3 mt-3">
                    <Button onClick={docsmsgdelete} disabled={sessionmessage.length === 0 || loadingdocsdelete} className="bg-cyan-500 dark:bg-white">{loadingdocsdelete ? <Spinner /> : <><RefreshCw />Reset Chat</>}</Button>
                    <div className="flex gap-2 items-center">
                        {serviceemail && (
                            <>
                                {docs.length > 0 ? (
                ...
                <div className="w-full bg-card mx-auto max-w-5xl rounded-2xl border p-3 shadow-lg">
                    <Textarea
                        disabled={Api.length === 0 || !model || !serviceemail || docs.length === 0 || loadingrecord || recordstatus}
                        value={input}
                    ...
                    <div className="flex items-center justify-between mt-2">
                        ...
                        <div className="flex gap-2">
                            ...
                            <Button
                                onClick={handleSend}
                                disabled={sending || !serviceemail || !model || !input.trim() || docs.length === 0 || loadingrecord || recordstatus}
                                size="icon"
                                className="bg-cyan-500 dark:bg-white rounded-full"
                            >
