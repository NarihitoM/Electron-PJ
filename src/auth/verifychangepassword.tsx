...
export const Verifychangepassword = () => {
    ...
    const {
        ...
        loadingpasswordchangeverify,
        loadingpasswordchangeresend,
        clearchangepassword
    } = userauthstore();
    ...
    const Changepasswordverify = async () => {
        try {
            const data = await changepasswordverify(
                stateid!,
                code
            );
            if (data.success) {
                ...
                setTimeout(() => {
                    navigate(`/passwordchange/${stateid}`, {
                        replace: true
                    })
                }, 1000);
            }
    ...
    const Resendchangepasswordverify = async () => {
        if (timer > 0) return;
        try {
            const data = await changepasswordresend(
                stateid!
            );
            if (data.success) {
                toast.success(data.message);
                        ...
                        <div className="w-full max-w-xs">
                            <div className="rounded-lg flex flex-col gap-4">
                                ...
                                <div className="flex items-center">
                                    ...
                                    <Button disabled={timer > 0 || loadingpasswordchangeresend} variant="link" onClick={Resendchangepasswordverify}>
                                        {loadingpasswordchangeresend ?
                                            <Spinner />
                                            : timer > 0 ? timer :
                                                "Resend"}</Button>
                                ...
                                <div className="flex gap-2 justify-end">
                                    <Button
                                        disabled={loadingpasswordchangeverify}
                                        onClick={Changepasswordverify}
                                        className="bg-cyan-500 dark:bg-white"
                                    >
                                        {loadingpasswordchangeverify ? <Spinner /> : "Verify"}
                                    </Button>
                                    <Button
                                        className="bg-cyan-500 dark:bg-white"
                                        onClick={() => {
