...
const backendUrl = import.meta.env.VITE_BACKEND_URL || process.env.BACKEND_URL || process.env.VITE_BACKEND_URL || "https://multimate-server.vercel.app";
...
Server.interceptors.response.use(
    ...
    async (error) => {
        ...
        if (error.response?.status === 429 && typeof window !== "undefined") {
            ...
            const message = error.response.data?.message || `Rate limited. Try again in ${retryAfter} seconds.`;
