  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen} className="w-full mb-2">
      <CollapsibleTrigger asChild>
        <Button
          variant="ghost"
          className="group flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium transition-all active:scale-95 text-muted-foreground"
        >
      ...
      <CollapsibleContent className="animate-in fade-in slide-in-from-top-1 duration-200 w-full">
        <div className="mt-1 flex flex-col rounded-xl border border-zinc-100 dark:border-zinc-800 bg-white dark:bg-black overflow-hidden shadow-sm">
          ...
          <div
            className="p-3 max-h-64 overflow-y-auto scrollbar-thin"
            style={{ scrollbarWidth: "none" }}
          >
            <pre
              className={`text-[10px] font-mono whitespace-pre-wrap ${isStreaming ? "text-foreground" : "text-muted-foreground"}`}
            >
              {thinking}
            </pre>
          </div>
