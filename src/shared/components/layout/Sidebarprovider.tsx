...
export const Sidebarprovider = () => {
    ...
    const handleItemClick = (type: string, isLoading: boolean) => {
        setNavItems(prevItems =>
            prevItems.map(item =>
                item.type === type ? { ...item, loading: isLoading } : item
            )
        );
    };
    ...
    const connectedAgentItems = useMemo(() => {
        return Agent.filter((item) => {
            switch (item.title) {
                ...
                case "Notion": return !!workspacename;
                case "GoogleSheet":
                case "GoogleDocs": return !!serviceemail;
                case "Telegram": return !!telegramUserdata;
                case "n8n": return n8nConnected;
                            ...
                            <SidebarGroupContent>
                                ...
                                {loadingServices ? (
                                    ...
                                    <div className="flex flex-col items-center gap-2 py-4 px-4 text-center">
                                        ...
                                        <Button size="sm" variant="outline" onClick={() => navigate("/app/settings")} className="text-cyan-500 dark:text-white">
                                            Connect a Service
                                        </Button>
                                    </div>
                                ) : (
                            ...
                            <SidebarGroupContent>
                                ...
                                {loadingchat ? (
                                    ...
                                    <div className="flex flex-col gap-2 items-center justify-center py-6 px-4 text-center">
                                        ...
                                        <Button size="sm" variant="outline" onClick={async () => { setchaterror(false); try { fetchchat(); } catch { setchaterror(true); } }}>Retry</Button>
                                    </div>
                                ) : chathistory.map((element) => (
                                        <SidebarMenu key={element.id}>
                                            <SidebarMenuItem>
                                                <SidebarMenuButton className={location.pathname.startsWith(`${element.url}`) ? "bg-linear-to-r from-card hover:text-cyan-500 active:text-cyan-500 from-30% to-cyan-300 text-cyan-500 dark:text-white/70 dark:to-white/20 justify-between flex" : "justify-between flex"} onClick={() => navigate(`${element.url}`)}>
                                    ...
                                    {chatLoadingMore && [1, 2].map((_, index) => (
