  ...
  return (
    <div className="relative group w-full my-4">
      <div className="flex items-center justify-between px-4 py-2 bg-cyan-200 dark:bg-zinc-800 rounded-t-xl">
        <span className="text-xs font-mono dark:text-white text-muted-foreground">{language}</span>
        <button onClick={copyToClipboard} className="p-1.5 rounded-md  ">
          {copied ? (
            <Check className="h-4 w-4 text-green-700" />
          ) : (
            <Copy className="h-4 w-4 dark:text-white text-muted-foreground" />
          )}
        </button>
      </div>
      <SyntaxHighlighter
...
const components = {
  ...
  th({ children, ...props }: any) {
    return (
      <th
        className="px-3 py-2 text-left font-semibold text-foreground border-b border-zinc-200 dark:border-zinc-700"
        {...props}
      >
        {children}
      </th>
  ...
  td({ children, ...props }: any) {
    return (
      <td className="px-3 py-2 border-b border-zinc-100 dark:border-zinc-800" {...props}>
        {children}
      </td>
    );
  },
