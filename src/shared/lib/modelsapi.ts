...
function inferCapabilities(id: string): string[] {
  ...
  if (
    /vl|vision|image|multimodal|visual|pixtral|sonnet|opus|haiku|gpt-4|gpt-5|gemini|claude/.test(
      lower,
    )
  )
    caps.push("vision");
  if (
    ...
    caps.push("code");
  if (/reason|think|o1|o3|o4|deepseek|nemotron.*reasoning|lfm.*thinking/.test(lower))
    caps.push("reasoning");
  if (
    /gpt|claude|gemini|mistral-large|mistral-medium|devstral|open-mistral|function|tool/.test(lower)
  )
    caps.push("tools");
  return caps;
}
    ...
    if (models.length === 0) {
      try {
        if (typeof window !== "undefined" && (window as any).api?.fetchOllamaModels) {
          const cached = datafetch.getQueryData<
            { id: number; provider: string; apiKey: string; host?: string }[]
          >(["key"]);
          const stored = cached?.find((s) => s.provider.toLowerCase() === "ollama");
          ...
          if (stored) {
            if (stored.host) {
              ...
              try {
                ...
                baseUrl = stored.apiKey;
              }
            }
          }
          const names = await (window as any).api.fetchOllamaModels(baseUrl, apiKey);
          models = (names || [])
            .map((n: string) => ({ model: n, capabilities: [] }))
            .map(ensureCapabilities);
        }
      } catch (err) {
        console.error(
          `Electron bridge model fetch failed for ollama:`,
