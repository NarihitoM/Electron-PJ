    ...
    return (
        <>
            <div className="flex w-full gap-2 justify-between mx-auto max-w-5xl mb-3 mt-3">
                <Button onClick={docsmsgdelete} disabled={store.sessionmessage_docs.length === 0 || loadingdocsdelete} className="bg-cyan-500 dark:bg-white">
                    ...
                    {serviceemail ? (
                        <>
                            ...
                            {docs.length > 0 ? (
                                ...
                                <Button onClick={() => store.setOpendocs(true)}
                        ...
                        <Button onClick={() => store.setOpenservice(true)}
                            className="bg-cyan-500 dark:bg-card-foreground dark:text-black"
                        >
                            Add Service Account
                        </Button>
            ...
            <Dialog open={store.opendocs} onOpenChange={store.setOpendocs} modal={false}>
                <DialogContent>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <Input id="docs" placeholder="Enter GoogleDocsUrl" value={store.docsinput} onChange={(e) => store.setDocsinput(e.target.value)} />
                    </div>
                    <DialogFooter>
                        <Button
                            onClick={adddocsurl}
                        ...
                        <Button variant="destructive" onClick={() => store.setOpendocs(false)}>Cancel</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
            ...
            <Dialog open={store.openservice} onOpenChange={store.setOpenservice} modal={false}>
                <DialogContent>
                    ...
                    <div className="flex flex-col gap-2">
                        ...
                        <Input id="email" placeholder="Enter Service Email" value={store.useremail_docs} onChange={(e) => store.setUseremail_docs(e.target.value)} />
                    </div>
                    <div className="flex flex-col gap-2">
                        <Label htmlFor="key">Service Key</Label>
                        <Input id="key" type="password" placeholder="Enter Service Key" value={store.key_docs} onChange={(e) => store.setKey_docs(e.target.value)} />
                    </div>
                    <DialogFooter>
                        <Button
                            onClick={addservice}
                        ...
                        <Button variant="destructive" onClick={() => store.setOpenservice(false)}>Cancel</Button>
                    </DialogFooter>
                </DialogContent>
            </Dialog>
        </>
