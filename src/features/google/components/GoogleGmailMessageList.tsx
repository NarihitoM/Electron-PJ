...
import { Googlegmailtool } from "@/shared/config/toolsselection";
...
import { useGoogleGmailMessages } from "../hooks/useGoogleGmailMessages";
import { toast } from "sonner";

export const GoogleGmailMessageList = () => {
  const { data: userdata } = useUser();
  const { data: googleService } = useGoogleService();
  const store = googleauthstore();
  const { connect, isChecking } = useGoogleConnect();
  const fetchGmailMessages = useGoogleGmailMessages();
  ...
  const loadMore = async () => {
    if (!store.nextCursor_gmail || !store.hasMore_gmail || store.loadingMore_gmail) return;
    store.setLoadingMore_gmail(true);
    try {
      const container = scrollContainerRef.current;
      const prevScrollHeight = container?.scrollHeight ?? 0;

      const response = await fetchGmailMessages(store.nextCursor_gmail);
      if (response.success && response.data) {
        const data = response.data;
        store.updateSessionMessages_gmail((prev) => [...(data.messages ?? []), ...prev]);
        store.setNextCursor_gmail(data.nextCursor);
        store.setHasMore_gmail(data.hasMore);
      ...
      store.setLoadingMore_gmail(false);
    }
  };
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && store.hasMore_gmail && !store.loadingMore_gmail) {
          loadMore();
        }
      },
      { threshold: 0.1 },
  ...
  }, [store.hasMore_gmail, store.loadingMore_gmail]);

  const fetchMessages = async () => {
    try {
      store.setloadingfetch_gmail(true);
      store.setloadingerror_gmail(false);
      store.setSending_gmail(false);

      store.setsessionmessage_gmail([]);
      store.setNextCursor_gmail(null);
      store.setHasMore_gmail(false);
      const response = await fetchGmailMessages();
      if (response.success && response.data) {
        store.setsessionmessage_gmail(response.data.messages ?? []);
        store.setNextCursor_gmail(response.data.nextCursor);
        store.setHasMore_gmail(response.data.hasMore);
      }
    } catch (err: unknown) {
      store.setloadingerror_gmail(true);
      if (err instanceof Error) {
        const Error = err as any;
        toast.error(Error.response?.data?.message || err.message);
      } else {
      ...
      store.setloadingfetch_gmail(false);
    }
  };
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-5xl py-5">
        {store.loadingfetch_gmail ? (
        ...
        ) : store.sessionmessage_gmail && store.sessionmessage_gmail.length > 0 ? (
          <>
            <div ref={topSentinelRef} />
            {store.loadingMore_gmail && (
            ...
            {store.sessionmessage_gmail.map((msg, index) => {
              const isUser = msg.role === "user";
              const isLastMessage = index === store.sessionmessage_gmail.length - 1;
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {!isUser && store.sending_gmail && isLastMessage && !msg.content && (
                      ...
                      <div className="flex flex-wrap gap-1 mb-1">
                        {msg.images.map((url, i) => (
                          <div
                            ...
                            onClick={() => {
                              store.setLightboxImages_gmail(msg.images!);
                              store.setLightboxIndex_gmail(i);
                              store.setLightboxOpen_gmail(true);
                            }}
                          >
                            <img src={url} alt="attached" className="w-full h-full object-cover" />
                            {store.uploadingImageUrls_gmail.has(url) && (
                      ...
                      <div className="flex flex-wrap gap-2 mb-2">
                        {msg.generatedImages.map((url, i) => (
                          <div
                            ...
                            onClick={() => {
                              store.setLightboxImages_gmail(msg.generatedImages!);
                              store.setLightboxIndex_gmail(i);
                              store.setLightboxOpen_gmail(true);
                            }}
                          >
                    ...
                    <div
                      className={`rounded-2xl leading-relaxed text-[15px] whitespace-pre-wrap w-full overflow-hidden wrap-break-word min-w-0 ${
                        isUser
                    ...
                    >
                      {msg.thinking && (
                        <ThinkingBlock
                          ...
                          isStreaming={
                            store.sending_gmail && index === store.sessionmessage_gmail.length - 1
                          }
                        />
                      )}
                      <motion.div
                      ...
                      >
                        <div className="grid grid-cols-1 gap-2 mb-1 w-fit">
                          {msg.toolsCall
                            ?.filter((t: any) => !t.isChain)
                            .map((tool: any) => (
                              <Collapsible key={tool.id} className="w-full space-y-2">
                                <CollapsibleTrigger asChild>
                                  <Button
                                    variant="ghost"
                                    className={`group flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium transition-all active:scale-95 ${
                                      tool.status === "done"
                                  ...
                                  >
                                    ...
                                    {tool.status === "loading" ? (
                                      ...
                                      >
                                        {Googlegmailtool[tool.name.toLowerCase()]}
                                      </motion.span>
                                    ) : (
                                      <span>{Googlegmailtool[tool.name.toLowerCase()]}</span>
                                    )}
                                    {tool.status !== "loading" && (
                                      <ChevronDown
                                        size={15}
                                  ...
                                  <div className="flex flex-col rounded-xl border border-zinc-100 dark:border-zinc-800 bg-background overflow-hidden shadow-sm max-w-[95%]">
                                    ...
                                    {tool.result && (
                                      <div className="flex-1 overflow-hidden">
                                        ...
                                        <div
                                          className="p-3 max-h-62.5 overflow-y-auto scrollbar-thin"
                                          style={{ scrollbarWidth: "none" }}
                                        >
                    ...
                    {msg.content && (
                      <button
                        onClick={() => {
                          ...
                          store.setCopiedIndex_gmail(index);
                          setTimeout(() => store.setCopiedIndex_gmail(null), 1500);
                        }}
                        className={`${store.copiedIndex_gmail === index ? "opacity-100" : "opacity-0 group-hover:opacity-100"} transition-opacity p-1 rounded self-end -mt-1`}
                      >
                        {store.copiedIndex_gmail === index ? (
          ...
          <div className="min-h-[50vh] flex flex-col gap-2 justify-center items-center">
            <h1 className="text-3xl">GoogleGmail Agenting</h1>
            <p className="text-sm text-muted-foreground">
              Send Message And Agent Will Handle Everything.
            </p>
