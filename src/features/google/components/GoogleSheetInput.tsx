...
export const GoogleSheetInput = () => {
    ...
    useEffect(() => {
        if (!store.provider) { store.setModelList_sheet([]); return }
            ...
            await googleauth.sendsheetmessage(
                ...
                (data: { thread_id: string; tool_calls: Array<{ id: string; name: string; query: Record<string, unknown> }> }) => {
                    ...
                    if (toolCall) {
                        ...
                        store.pendingApprovalRef_sheet.current = { name: toolCall.name, query: toolCall.query ?? null }
    ...
    const isDisabled = Api.length === 0 || !store.provider || !store.model || !serviceemail || sheet.length === 0 || store.loadingrecord_sheet || store.recordstatus_sheet || store.uploadingImages_sheet
    ...
    const renderTypeBadge = () => {
        ...
        return (
            <button
                onClick={() => { store.settype_sheet("text"); store.setHover_sheet(false) }}
    ...
    return (
        <div className="w-full bg-card mx-auto max-w-5xl rounded-2xl border p-3 shadow-lg">
            ...
            <Textarea
                ...
                placeholder={store.recordstatus_sheet ? "Listening..." : store.loadingrecord_sheet ? "Transcribing..." : "Message..."}
                onKeyDown={(e) => {
            ...
            <div className="flex items-center justify-between mt-2">
                ...
                <div className="flex gap-2">
                    {Api.length > 0 && (
                        <ModelSelect modelList={store.modelList_sheet} provider={store.provider || ""} model={store.model} loading={store.modelsLoading_sheet} disabled={!store.provider} onSelect={store.setModel} reasoningLevel={store.reasoningLevel_sheet} onReasoningLevelChange={store.setReasoningLevel_sheet} />
                    )}
                    <Button
                        disabled={store.loadingrecord_sheet || !serviceemail || sheet.length === 0 || !store.model || !store.provider}
                        onClick={store.recordstatus_sheet ? stopRecording : startRecording}
                        size="icon"
                        className="bg-cyan-500 dark:bg-white rounded-full">
                        {store.recordstatus_sheet ? <Square size={14} className="fill-current" /> :
                            store.loadingrecord_sheet ? <Spinner /> : <Mic size={14} />}
                    </Button>
                    <Button
                        onClick={store.sending_sheet ? () => abortControllerRef.current?.abort() : handleSend}
                        disabled={!store.sending_sheet && (!store.provider || !store.model || (!store.input_sheet.trim() && store.pendingImages_sheet.length === 0) || sheet.length === 0 || store.loadingrecord_sheet || store.recordstatus_sheet || store.uploadingImages_sheet)}
                        size="icon"
                        className={store.sending_sheet ? "bg-red-500 hover:bg-red-600 rounded-full" : "bg-cyan-500 dark:bg-white rounded-full"}
                    >
                        {store.sending_sheet ? <Square size={16} className="fill-current" /> : <ArrowUp size={16} />}
                    </Button>
                </div>
            </div>
        </div>
