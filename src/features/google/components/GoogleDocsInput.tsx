...
export const GoogleDocsInput = () => {
    ...
    const isDisabled = Api.length === 0 || !store.provider || !store.model || !serviceemail || docs.length === 0 || loadingrecord || recordstatus || store.uploadingImages_docs
    ...
    return (
        <>
            <div className="flex w-full gap-2 justify-between mx-auto max-w-5xl mb-3 mt-3">
                <Button onClick={deleteDocsMessage} disabled={store.sessionmessage_docs.length === 0} className="bg-cyan-500 dark:bg-white">
                    {<><svg className="h-4 w-4 mr-1" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 2v6H3V2M3 8l1.5 14h15L21 8M10 12v4M14 12v4M7.5 2L9 4h6l1.5-2"/></svg>Reset Chat</>}
                </Button>
            </div>
                ...
                <div className="flex items-center justify-between mt-2">
                    <div className="flex gap-2 items-center">
                        ...
                        {store.type_docs && store.type_docs !== "text" && store.type_docs !== "create" && (
                            <button
                                onClick={() => { store.settype_docs("text"); }}
                    ...
                    <div className="flex gap-2">
                        {Api.length > 0 && (
                            <ModelSelect modelList={modelList} provider={store.provider || ""} model={store.model} loading={modelsLoading} disabled={!store.provider} onSelect={store.setModel} reasoningLevel={store.reasoningLevel_docs} onReasoningLevelChange={store.setReasoningLevel_docs} />
                        )}
                        <Button
                            disabled={loadingrecord || !serviceemail || docs.length === 0 || !store.model || !store.provider}
                        ...
                        <Button
                            ...
                            disabled={!store.sending_docs && (!serviceemail || !store.provider || !store.model || (!store.input_docs.trim() && store.pendingImages_docs.length === 0) || docs.length === 0 || loadingrecord || recordstatus || store.uploadingImages_docs)}
                            size="icon"
                            className={store.sending_docs ? "bg-red-500 hover:bg-red-600 rounded-full" : "bg-cyan-500 dark:bg-white rounded-full"}
                        >
                            {store.sending_docs ? <Square size={16} className="fill-current" /> : <ArrowUp size={16} />}
                        </Button>
                    </div>
                </div>
            </div>
