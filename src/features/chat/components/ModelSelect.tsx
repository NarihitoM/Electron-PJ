  ...
  return (
    <Tooltip>
      <TooltipTrigger
        render={
          <span
            className={`inline-flex items-center justify-center w-4 h-4 rounded-sm bg-white/10 ${cap.color}`}
          />
        }
      >
        <Icon size={10} />
...
function ReasoningLevelSelect({
  ...
  return (
    <Select
      value={value || "auto"}
      onValueChange={(v) => onChange(v === "auto" ? "" : (v as ReasoningLevel))}
    >
              ...
              <div className="max-h-60 overflow-y-auto p-1">
                {modelList.length === 0 && !loading && (
                  <div className="px-3 py-2 text-sm text-muted-foreground">
                    No models available. Configure your API key in Settings.
                  </div>
                )}
                {modelList.filter((entry) =>
                  ...
                  modelList
                    .filter((entry) => entry.model.toLowerCase().includes(search.toLowerCase()))
                    .map((entry) => (
                      <button
                        key={entry.model}
                        type="button"
