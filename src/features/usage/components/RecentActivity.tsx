...
export const RecentActivity = () => {
  ...
  const getPageNumbers = () => {
    ...
    if (totalPages <= 5) {
      ...
      for (
        let i = Math.max(2, recentPage - 1);
        i <= Math.min(totalPages - 1, recentPage + 1);
        i++
      ) {
        pages.push(i);
      }
                        ...
                        <TableRow key={r.id}>
                          <TableCell>
                            <div className="flex items-center gap-2">
                              {BRAND_ASSETS[r.provider] && (
                                <img
                                  src={BRAND_ASSETS[r.provider]}
                          ...
                          <TableCell className="text-sm font-mono">
                            {r.model.length > 20 ? r.model.slice(0, 20) + "..." : r.model}
                          </TableCell>
                          <TableCell>
                          ...
                          <TableCell className="text-right text-sm font-medium">
                            {r.provider === "zen" ? "Free" : formatCost(r.cost)}
                          </TableCell>
                          <TableCell className="text-right text-sm">
              ...
              {totalPages > 1 && (
                <div className="flex items-center justify-between mt-4">
                  <p className="text-xs text-muted-foreground">
                    Showing {(recentPage - 1) * recentLimit + 1}–
                    {Math.min(recentPage * recentLimit, recentTotal)} of {recentTotal}
                  </p>
                  <div className="flex items-center gap-1">
                    ...
                    {getPageNumbers().map((p, i) =>
                      p === "..." ? (
                        ...
                        <Button
                          ...
                          className={
                            recentPage === p
                              ? "bg-cyan-500 text-white dark:bg-white dark:text-black"
                              : ""
                          }
                    ...
                    <Button
                      size="icon"
                      variant="outline"
                      onClick={() => setRecentPage(Math.min(totalPages, recentPage + 1))}
