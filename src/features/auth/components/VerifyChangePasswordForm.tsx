    ...
    const handleVerify = async () => {
        try {
            ...
            if (data.success) {
                ...
                setTimeout(() => { navigate(`/passwordchange/${stateid}`, { replace: true }); }, 1000);
            }
        } catch (err: unknown) {
    ...
    return (
        <div className="rounded-lg flex flex-col gap-4">
            <div className="flex flex-col gap-2 items-center">
                ...
                <p className="text-balance text-muted-foreground">We have send 6 digits code to your email.</p>
            </div>
            ...
            <div className="flex gap-2 justify-end">
                <Button disabled={loadingverify} onClick={handleVerify} className="bg-cyan-500 dark:bg-white">
                    {loadingverify ? <Spinner /> : "Verify"}
                </Button>
                <Button className="bg-cyan-500 dark:bg-white" onClick={() => { userauthapi.clearpasswordchange(stateid); navigate("/signup", { replace: true }); }}>
                    Cancel
                </Button>
            </div>
        </div>
