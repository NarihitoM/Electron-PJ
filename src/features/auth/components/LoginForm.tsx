    ...
    return (
        <div className="rounded-lg flex flex-col gap-4">
            ...
            <div className="flex flex-col gap-2 mt-5">
                ...
                <Input value={useremail} onChange={(e) => setuseremail(e.target.value)} id="Useremail" placeholder="Enter Email" />
            </div>
            <div className="flex flex-col gap-1 relative">
                <button type="button" onClick={() => setshow(prev => !prev)} className="absolute right-3 top-10.5 text-muted-foreground">
                    {!show ? <EyeOff size={18} /> : <Eye size={18} />}
                </button>
                <div className="flex items-center justify-between">
                    <Label htmlFor="password" title="Password" className="flex items-center gap-2"><Lock size={15} /> Password</Label>
                    <Button type="button" variant="link" onClick={() => navigate("/emailverify")}>Forgot password?</Button>
                </div>
            ...
            <div className="w-full">
                <Button disabled={loading} onClick={handleLogin} className="bg-cyan-500 dark:bg-white w-full">
            ...
            <div className="grid grid-cols-1 w-full gap-4">
                <Button onClick={handleGoogleLogin} disabled={loadinggoogle} variant="outline" className="w-full rounded-full">
                    {loadinggoogle ? <Spinner /> : <><GoogleIcon /> Google</>}
                </Button>
            </div>
            <div className="w-full gap-2 flex justify-center items-center mt-5">
                <Checkbox id="terms" checked={isAgreed} className="checked:bg-cyan-500 dark:checked:bg-white" onCheckedChange={(checked) => setIsAgreed(checked === true)} />
                <div className="flex items-center text-[12px] text-muted-foreground">
                    <label htmlFor="terms" className="cursor-pointer">I have read and agree to the</label>
                    <a href="#" target="_blank" rel="noopener noreferrer" className="ml-1 text-cyan-500 dark:text-white hover:underline font-medium">terms and conditions.</a>
                </div>
            </div>
        </div>
    );
