...
import { vercelauthstore, initialVercelCron } from "../store/store";
...
export const VercelCronScheduler = () => {
  ...
  useEffect(() => {
    const fetchCron = async () => {
      try {
        const response = await vercelauth.vercelcronget();
        if (response.success && response.data) {
          store.setVercelcron(response.data);
        } else {
          store.setVercelcron({ ...initialVercelCron });
        }
      } catch {
        store.setVercelcron({ ...initialVercelCron });
      }
  ...
  useEffect(() => {
    if (store.vercelcron.customSchedule) {
      try {
        const schedule = JSON.parse(store.vercelcron.customSchedule);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [store.vercelcron.customSchedule]);

  useEffect(() => {
    if (!store.vercelcron.provider) {
      setCronModelList([]);
      return;
    }
    getProviderModels(store.vercelcron.provider).then((models) => {
      setCronModelList(models);
    });
  }, [store.vercelcron.provider]);

  const handlecronchange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>,
  ) => {
    const { name, value } = e.target;
    store.setVercelcron({ ...store.vercelcron, [name]: value });
  };
  ...
  const cronsubmint = async () => {
    try {
      ...
      const payload = {
        ...
        customSchedule:
          store.vercelcron.crontype === "custom"
            ? JSON.stringify({
      ...
      const response = await vercelauth.vercelcroncreate(payload);
      if (response.success) {
  ...
  return (
    <Dialog open={store.opencron} onOpenChange={store.setOpencron} modal={false}>
      <DialogContent className="max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create Vercel Cron Task</DialogTitle>
          <DialogDescription>
            Configure your AI Agent to work on your Vercel projects automatically on a recurring
            schedule.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 pt-2">
            ...
            <div className="space-y-1">
              ...
              {apiWithLogos.length > 0 ? (
                <div className="flex gap-2">
                  <Select
                    value={store.vercelcron.provider || undefined}
                    disabled={!store.vercelcron.isActive}
                    onValueChange={(value) => {
                      store.setVercelcron({ ...store.vercelcron, provider: value ?? "" });
                    }}
                  >
                    <SelectTrigger className="w-full flex items-center gap-2">
                      {store.vercelcron.provider ? (
                        <div className="flex items-center gap-2">
                          <img
                            src={BRAND_ASSETS[store.vercelcron.provider.toLowerCase()]}
                            className="bg-white rounded-lg p-0.5 w-5 h-5 object-contain shrink-0"
                            alt=""
                          />
                          <span>{getProviderDisplayName(store.vercelcron.provider)}</span>
                        </div>
                      ) : (
                        "Select Provider"
                      )}
              ...
              {apiWithLogos.length > 0 && (
                <div className="relative overflow-visible">
                  <Button
                    ...
                    disabled={!store.vercelcron.provider || !store.vercelcron.isActive}
                  >
                    {store.vercelcron.model ? (
                      <div className="flex items-center gap-2">
                        <img
                          src={getProviderImage(store.vercelcron.provider || "")}
                          className="bg-white rounded-lg p-0.5 w-5 h-5 object-contain shrink-0"
                        />
                        <span className="truncate">{store.vercelcron.model}</span>
                      </div>
                    ) : (
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                <Button
                  key={opt.value}
                  type="button"
                  variant={store.vercelcron.crontype === opt.value ? "default" : "outline"}
                  onClick={() => store.setVercelcron({ ...store.vercelcron, crontype: opt.value })}
                  disabled={!store.vercelcron.isActive}
                  className={
                    store.vercelcron.crontype === opt.value
          ...
          {store.vercelcron.crontype === "custom" && store.vercelcron.isActive && (
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              name="triggerAt"
              type="time"
              value={store.vercelcron.triggerAt}
          ...
          <div className="space-y-1">
            ...
            <Textarea
              id="message"
              name="message"
              value={store.vercelcron.message}
              onChange={handlecronchange}
              disabled={!store.vercelcron.isActive}
              placeholder="What should the agent do on your Vercel projects?"
              className="min-h-20"
              required
            />
          </div>
          ...
          <DialogFooter className="flex items-center pt-2">
            <div className="flex">
              ...
              <Switch
                checked={store.vercelcron.isActive}
                onCheckedChange={(checked) => {
                  store.setVercelcron({ ...store.vercelcron, isActive: checked });
                }}
