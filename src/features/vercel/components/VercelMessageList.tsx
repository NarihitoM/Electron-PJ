...
import { Verceltool } from "@/shared/config/toolsselection";
import { extractToolMessage } from "@/shared/utils/toolutils";
import { useUser } from "@/features/auth/hooks/useUser";
import { useVercelAccount } from "../hooks/useVercelAccount";
import { useVercelMessages } from "../hooks/useVercelMessages";
import { vercelauthstore } from "../store/store";
import { toast } from "sonner";

export const VercelMessageList = () => {
  const { data: userdata } = useUser();
  const { isLoading: accountLoading } = useVercelAccount();
  const store = vercelauthstore();
  const fetchVercelMessages = useVercelMessages();
  ...
  const fetchMessages = async () => {
    ...
    try {
      const response = await fetchVercelMessages();
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const response = await fetchVercelMessages(store.nextCursor);
      if (response.success && response.data) {
  ...
  const isLoading = accountLoading || store.loadingfetch;
  ...
  if (!store.sessionmessage || store.sessionmessage.length === 0) {
    return (
      <div
        ref={scrollContainerRef}
        className="flex-1 overflow-y-auto mt-4"
      ...
      >
        <div className="mx-auto max-w-5xl py-5">
          <div className="min-h-[50vh] flex flex-col gap-2 justify-center items-center">
            ...
            <p className="text-sm text-muted-foreground">
              Ask your AI agent to manage your Vercel projects and deployments.
            </p>
          </div>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              >
                  ...
                  <div className="flex flex-wrap gap-1 mb-1">
                    {msg.images.map((url, i) => (
                      <div
                        ...
                        onClick={() => {
                          store.setLightboxImages(msg.images!);
                          store.setLightboxIndex(i);
                          store.setLightboxOpen(true);
                        }}
                      >
                  ...
                  <div className="flex flex-wrap gap-2 mb-2">
                    {msg.generatedImages.map((url, i) => (
                      <div
                        ...
                        onClick={() => {
                          store.setLightboxImages(msg.generatedImages!);
                          store.setLightboxIndex(i);
                          store.setLightboxOpen(true);
                        }}
                      >
                  ...
                  >
                    {msg.thinking && (
                      <ThinkingBlock
                        ...
                        isStreaming={store.sending && index === store.sessionmessage.length - 1}
                      />
                    )}
                            ...
                            <CollapsibleTrigger asChild>
                              ...
                              >
                                ...
                                {tool.status === "loading" ? (
                                  ...
                                  >
                                    {Verceltool[tool.name.toLowerCase()] || tool.name}
                                  </motion.span>
                                ) : (
                                  <span>{Verceltool[tool.name.toLowerCase()] || tool.name}</span>
                                )}
                                {tool.status !== "loading" && (
                                  <ChevronDown
                                    size={15}
                ...
                {msg.content && (
                  <button
                    onClick={() => onCopyMessage(index, msg.content)}
