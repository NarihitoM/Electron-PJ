...
import { VercelCronScheduler } from "./VercelCronScheduler";
...
export const VercelInput = () => {
  ...
  const deletemessages = async () => {
    try {
      ...
      const response = await deleteVercelMessage();
      if (response.success) {
      ...
      <div className="flex w-full gap-2 justify-between mx-auto max-w-5xl mb-3 mt-3">
        <div className="flex gap-2">
          <Button
            ...
            disabled={store.sessionmessage.length === 0 || store.loadingdeletemsg}
            className="bg-cyan-500 dark:bg-white"
          >
            {store.loadingdeletemsg ? (
              <Spinner />
            ) : (
              <>
                <RefreshCw />
        ...
        <div className="flex items-center justify-between mt-2">
          <div className="flex gap-2 items-center">
            <ImagePicker
              ...
              uploading={store.uploadingImages}
              disabled={
                !connected || !store.model || !store.provider || loadingrecord || recordstatus
              }
              maxImages={4}
            />
            {connected && (
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" side="top" className="w-45">
                  ...
                  <DropdownMenuItem onClick={() => store.settype("projects")}>
                    <Box /> List Projects
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => store.settype("deployments")}>
                    <Box /> List Deployments
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => store.settype("logs")}>
                    <Box /> Deployment Logs
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => store.settype("redeploy")}>
                    <Box /> Redeploy
                  </DropdownMenuItem>
                  <DropdownMenuItem onClick={() => store.settype("env")}>
                    <Box /> Manage Env Vars
                  </DropdownMenuItem>
                </DropdownMenuContent>
              </DropdownMenu>
