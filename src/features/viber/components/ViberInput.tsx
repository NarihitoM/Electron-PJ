...
export const ViberInput = () => {
  ...
  const viberUserId = (viberAccount as any)?.viberUserId ?? "";
  ...
  const deletevibermessage = async () => {
    try {
      setLoadingviberdelmsg(true);
      const response = await viberauth.deletevibermsg();
      if (response.success) {
  ...
  return (
    <>
      ...
      <div className="w-full bg-card mx-auto max-w-5xl rounded-2xl border p-3 shadow-lg">
        ...
        <Textarea
          disabled={
            Api.length === 0 || !viberUserId || !model || !provider || loadingrecord || recordstatus
          }
          value={input}
