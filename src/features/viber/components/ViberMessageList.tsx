...
export const ViberMessageList = () => {
  ...
  const loadMore = async () => {
    ...
    try {
      ...
      const response = await viberauth.fetchvibermessage(nextCursor);
  ...
  const fetchMessages = async () => {
    try {
      ...
      setNextCursor(null);
      setHasMore(false);
      const response = await viberauth.fetchvibermessage();
      if (response.success && response.data) {
    ...
    return (
      ...
      >
        <div className="mx-auto max-w-5xl py-5 min-h-[50vh] flex flex-col gap-3 justify-center items-center">
          ...
          <Button onClick={fetchMessages} className="bg-[#7360F2] text-white">
            Retry
          </Button>
        </div>
      </div>
        ...
        {loadingMore && (
          <div className="flex justify-center py-4">
            <div className="flex items-center gap-2 text-sm text-muted-foreground">
              <Spinner className="h-5 w-5 text-[#7360F2]" />
              Loading...
            </div>
          </div>
        )}
              ...
              <div className="flex h-8 w-8 items-center justify-center rounded-full mt-1">
                {isUser ? (
                  <Avatar className="h-8 w-8">
                    ...
                    />
                    <AvatarFallback className="bg-[#7360F2] border text-white">
                  ...
                  >
                    {msg.thinking && (
                      <ThinkingBlock
                        ...
                        isStreaming={sending && index === sessionmessage.length - 1}
                      />
                    )}
                        ...
                        .map((tool) => (
                          <Collapsible key={tool.id} className="w-full space-y-2">
                            <CollapsibleTrigger asChild>
                              ...
                              >
                                {tool.status === "loading" && (
                                  <Spinner className="w-4 h-4 animate-spin text-[#7360F2]" />
                                )}
                                {tool.status === "done" && (
                                ...
                                {tool.status === "loading" ? (
                                  ...
                                  >
                                    {Vibertool[tool.name.toLowerCase()] ?? tool.name}
                                  </motion.span>
                                ) : (
                                  <span>{Vibertool[tool.name.toLowerCase()] ?? tool.name}</span>
                                )}
                                {tool.status !== "loading" && (
                                  <ChevronDown
                                    size={15}
