    ...
    <>
      <div className="flex w-full gap-2 justify-between mx-auto max-w-5xl mb-3 mt-3">
        <div className="flex gap-2">
          <Button
            onClick={deleteslackmessage}
            disabled={sessionmessage.length === 0 || loadingslackdelmsg}
          ...
          {workspace && (
            <>
              {(publichannel.length > 0 ||
                ...
                >
                  <SelectTrigger>
                    <span className="truncate">{mode ? mode : "Select Channel"}</span>
                  </SelectTrigger>
                  <SelectContent
      ...
      <div className="w-full bg-card mx-auto max-w-5xl rounded-2xl border p-3 shadow-lg">
        ...
        <Textarea
          disabled={
            Api.length === 0 || !workspace || !model || !provider || loadingrecord || recordstatus
          }
          value={input}
        ...
        <div className="flex items-center justify-between mt-2">
          <div className="flex gap-2 items-center">
            ...
            {type === "getuser" && (
              <ToolButton label="Get Userinfo" settype={settype} sending={sending} />
            )}
            {type === "getteam" && (
              <ToolButton label="Get Teaminfo" settype={settype} sending={sending} />
            )}
          </div>
          <div className="flex gap-2">
            {Api.length > 0 && (
