...
import { n8nauthstore, initialN8nCron } from "../store/store";
...
export const N8nCronScheduler = () => {
  ...
  useEffect(() => {
    const fetchCron = async () => {
      try {
        const response = await n8nauth.n8ncronget();
        if (response.success && response.data) {
          store.setN8ncron(response.data);
        } else {
          store.setN8ncron({ ...initialN8nCron });
        }
      } catch {
        store.setN8ncron({ ...initialN8nCron });
      }
  ...
  useEffect(() => {
    if (store.n8ncron.customSchedule) {
      try {
        const schedule = JSON.parse(store.n8ncron.customSchedule);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [store.n8ncron.customSchedule]);

  useEffect(() => {
    if (!store.n8ncron.provider) {
      setCronModelList([]);
      return;
    }
    getProviderModels(store.n8ncron.provider).then((models) => {
      setCronModelList(models);
    });
  }, [store.n8ncron.provider]);

  const handlecronchange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>,
  ) => {
    const { name, value } = e.target;
    store.setN8ncron({ ...store.n8ncron, [name]: value });
  };
  ...
  const cronsubmint = async () => {
    try {
      ...
      const response = await n8nauth.n8ncroncreate(payload);
      if (response.success) {
  ...
  return (
    <Dialog open={store.opencron} onOpenChange={store.setOpencron} modal={false}>
      <DialogContent className="max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create n8n Cron Task</DialogTitle>
          <DialogDescription>
            Configure your AI Agent to work on your n8n instance automatically on a recurring
            schedule.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4 pt-2">
            ...
            <div className="space-y-1">
              ...
              {apiWithLogos.length > 0 ? (
                <div className="flex gap-2">
                  <Select
                    value={store.n8ncron.provider || undefined}
                    disabled={!store.n8ncron.isActive}
                    onValueChange={(value) => {
                      store.setN8ncron({ ...store.n8ncron, provider: value ?? "" });
                    }}
                  >
                    <SelectTrigger className="w-full flex items-center gap-2">
                      {store.n8ncron.provider ? (
                        <div className="flex items-center gap-2">
                          <img
                            src={BRAND_ASSETS[store.n8ncron.provider.toLowerCase()]}
                            className="bg-white rounded-lg p-0.5 w-5 h-5 object-contain shrink-0"
                            alt=""
                          />
                          <span>{getProviderDisplayName(store.n8ncron.provider)}</span>
                        </div>
                      ) : (
                        "Select Provider"
                      )}
              ...
              {apiWithLogos.length > 0 && (
                <div className="relative overflow-visible">
                  <Button
                    ...
                    disabled={!store.n8ncron.provider || !store.n8ncron.isActive}
                  >
                    {store.n8ncron.model ? (
                      <div className="flex items-center gap-2">
                        <img
                          src={getProviderImage(store.n8ncron.provider || "")}
                          className="bg-white rounded-lg p-0.5 w-5 h-5 object-contain shrink-0"
                        />
                        <span className="truncate">{store.n8ncron.model}</span>
                      </div>
                    ) : (
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                <Button
                  key={opt.value}
                  type="button"
                  variant={store.n8ncron.crontype === opt.value ? "default" : "outline"}
                  onClick={() => store.setN8ncron({ ...store.n8ncron, crontype: opt.value })}
                  disabled={!store.n8ncron.isActive}
                  className={
                    store.n8ncron.crontype === opt.value
          ...
          {store.n8ncron.crontype === "custom" && store.n8ncron.isActive && (
          ...
          <div className="space-y-1">
            ...
            <Textarea
              id="message"
              name="message"
              value={store.n8ncron.message}
              onChange={handlecronchange}
              disabled={!store.n8ncron.isActive}
              placeholder="What should the agent do on your n8n instance?"
              className="min-h-20"
              required
            />
          </div>
          ...
          <DialogFooter className="flex items-center pt-2">
            <div className="flex">
              ...
              <Switch
                checked={store.n8ncron.isActive}
                onCheckedChange={(checked) => {
                  store.setN8ncron({ ...store.n8ncron, isActive: checked });
                }}
