...
export const VideoAnalysisResult = () => {
  const { summary, timestamps, isPending, loadingupload, analysisError, generateTranscript } =
    videoauthstore();
            ...
            {timestamps.length > 0 &&
              timestamps.map((segment) => (
                ...
                >
                  ...
                  <p className="text-sm leading-relaxed transition-colors pt-0.5">{segment.text}</p>
                </div>
              ))}
          </div>
        </div>
  ...
  if (analysisError) {
    return (
      <div className="h-screen w-full flex flex-col justify-center items-center text-center gap-2">
        ...
        <Button onClick={() => generateTranscript?.()} className="bg-cyan-500 dark:bg-white mt-2">
          Retry
        </Button>
      </div>
    );
  ...
  return (
    <div className="h-screen w-full flex flex-col justify-center items-center text-center gap-1">
      ...
      <p className="text-[11px] text-muted-foreground">
        Generate the video to transcript and summarize.
      </p>
    </div>
  );
};
