...
import { discordauthstore, initialDiscordCron } from "../store/store";
...
export const DiscordCronScheduler = () => {
  const { data: Api = [] } = useServiceKeys();
  const { data: accountData } = useDiscordAccount();
  const store = discordauthstore();
  ...
  const guildId = (accountData as any)?.guildId ?? "";
  const { data: channels = [] } = useDiscordChannels(!!guildId);
  ...
  useEffect(() => {
    const fetchCron = async () => {
      try {
        const response = await discordauth.discordcronget();
        if (response.success && response.data) {
          store.setDiscordcron(response.data);
        } else {
          store.setDiscordcron({ ...initialDiscordCron });
        }
      } catch {
        store.setDiscordcron({ ...initialDiscordCron });
      }
  ...
  useEffect(() => {
    if (store.discordcron.customSchedule) {
      try {
        const schedule = JSON.parse(store.discordcron.customSchedule);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [store.discordcron.customSchedule]);

  useEffect(() => {
    if (!store.discordcron.provider) {
      setCronModelList([]);
      return;
    }
    getProviderModels(store.discordcron.provider).then((models) => {
      setCronModelList(models);
    });
  }, [store.discordcron.provider]);

  const handlecronchange = (
    e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>,
  ) => {
    const { name, value } = e.target;
    store.setDiscordcron({ ...store.discordcron, [name]: value });
  };
  ...
  const cronsubmint = async () => {
    try {
      ...
      const payload = {
        ...
        customSchedule:
          store.discordcron.crontype === "custom"
            ? JSON.stringify({
      ...
      const response = await discordauth.discordcroncreate(payload);
      if (response.success) {
  ...
  return (
    <Dialog open={store.opencron} onOpenChange={store.setOpencron} modal={false}>
      <DialogContent className="max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Create Discord Cron Task</DialogTitle>
          <DialogDescription>
        ...
        <div className="space-y-4 pt-2">
          <div className="space-y-1">
            ...
            {connected && (
              <Select
                value={store.discordcron.channelId || undefined}
                disabled={!store.discordcron.provider || !store.discordcron.isActive}
                onValueChange={(val) => {
                  const channel = textChannels.find((c) => c.id === val);
                  store.setDiscordcron({
                    ...store.discordcron,
                    channelId: val ?? "",
              ...
              >
                <SelectTrigger className="w-full">
                  <span className="truncate">
                    {store.discordcron.channelId
                      ? `#${textChannels.find((c) => c.id === store.discordcron.channelId)?.name ?? store.discordcron.channelName}`
                      : "Select Channel"}
                  </span>
                </SelectTrigger>
            ...
            <div className="space-y-1">
              ...
              {apiWithLogos.length > 0 ? (
                <div className="flex gap-2">
                  <Select
                    value={store.discordcron.provider || undefined}
                    disabled={!store.discordcron.isActive}
                    onValueChange={(value) => {
                      store.setDiscordcron({ ...store.discordcron, provider: value ?? "" });
                    }}
                  >
                    <SelectTrigger className="w-full flex items-center gap-2">
                      {store.discordcron.provider ? (
                        <div className="flex items-center gap-2">
                          <img
                            src={BRAND_ASSETS[store.discordcron.provider.toLowerCase()]}
                            className="bg-white rounded-lg p-0.5 w-5 h-5 object-contain shrink-0"
                            alt=""
                          />
                          <span>{getProviderDisplayName(store.discordcron.provider)}</span>
                        </div>
                      ) : (
                        "Select Provider"
                      )}
              ...
              {apiWithLogos.length > 0 && (
                <div className="relative overflow-visible">
                  <Button
                    ...
                    disabled={!store.discordcron.provider || !store.discordcron.isActive}
                  >
                    {store.discordcron.model ? (
                      <div className="flex items-center gap-2">
                        <img
                          src={getProviderImage(store.discordcron.provider || "")}
                          className="bg-white rounded-lg p-0.5 w-5 h-5 object-contain shrink-0"
                        />
                        <span className="truncate">{store.discordcron.model}</span>
                      </div>
                    ) : (
            ...
            <div className="flex gap-2 flex-wrap">
              {[
                ...
                <Button
                  key={opt.value}
                  type="button"
                  variant={store.discordcron.crontype === opt.value ? "default" : "outline"}
                  onClick={() =>
                    store.setDiscordcron({ ...store.discordcron, crontype: opt.value })
                  }
                  disabled={!store.discordcron.isActive}
                  className={
                    store.discordcron.crontype === opt.value
          ...
          {store.discordcron.crontype === "custom" && store.discordcron.isActive && (
          ...
          <div className="space-y-1">
            ...
            <Input
              ...
              name="triggerAt"
              type="time"
              value={store.discordcron.triggerAt}
          ...
          <div className="space-y-1">
            ...
            <Textarea
              id="message"
              name="message"
              value={store.discordcron.message}
              onChange={handlecronchange}
              disabled={!store.discordcron.isActive}
          ...
          <DialogFooter className="flex items-center pt-2">
            <div className="flex">
              ...
              <Switch
                checked={store.discordcron.isActive}
                onCheckedChange={(checked) => {
                  store.setDiscordcron({ ...store.discordcron, isActive: checked });
                }}
