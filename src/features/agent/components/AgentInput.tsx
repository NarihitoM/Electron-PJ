...
export const AgentInput = () => {
    ...
    const onAbortWorkflow = () => {
        ...
        store.workflowGenRef.current++
        store.setNodes((prev) => prev.map((n: any) => ({ ...n, status: "idle" as const, activeTool: null })))
    }
            ...
            <div className="flex items-center justify-end gap-2 mb-2 flex-wrap px-1">
                <Select onValueChange={(val) => store.setType(val ?? "")} value={store.type}>
                    <SelectTrigger className="w-40">
                        <span className="truncate">{store.type ? store.type.substring(0, 15) + "..." : "Select Mode"}</span>
                    </SelectTrigger>
                ...
                {store.type === "Specific Node" && (
                    <Select onValueChange={(val) => val && store.setSelectnode(val)} value={store.selectnode ?? ""} disabled={!store.type}>
                        <SelectTrigger className="w-44">
                            <span className="truncate">{store.selectnode ? store.selectnode.substring(0, 15) + "..." : "Select Agent Node"}</span>
                        </SelectTrigger>
                        <SelectContent className="p-1 w-60">
                            {agents.map((n: any) => <SelectItem key={n.id} value={n.name}>{n.name}</SelectItem>)}
                        </SelectContent>
                    </Select>
                )}
                {store.type === "Range Node" && (
                    agents.length > 1 ? (
                        <div className="flex gap-2">
                            <Select onValueChange={(val) => val && store.setFirstnode(val)} value={store.firstnode ?? ""} disabled={!store.type}>
                                <SelectTrigger className="w-44">
                                    <span className="truncate">{store.firstnode ? store.firstnode.substring(0, 15) + "..." : "First Node"}</span>
                                </SelectTrigger>
                                <SelectContent className="p-1 w-60">
                                    {agents.map((n: any) => <SelectItem key={n.id} value={n.name}>{n.name}</SelectItem>)}
                                </SelectContent>
                            </Select>
                            <Select onValueChange={(val) => val && store.setLastnode(val)} value={store.lastnode ?? ""} disabled={!store.type}>
                                <SelectTrigger className="w-44">
                                    <span className="truncate">{store.lastnode ? store.lastnode.substring(0, 15) + "..." : "Last Node"}</span>
                                </SelectTrigger>
                                <SelectContent className="p-1 w-60">
                                    {agents.map((n: any) => <SelectItem key={n.id} value={n.name}>{n.name}</SelectItem>)}
                                </SelectContent>
                            ...
                            <Sheet open={store.servicesOpen} onOpenChange={store.setServicesOpen}>
                                ...
                                <SheetContent side="right" className="w-100 sm:w-135">
                                    <SheetHeader>
                                        ...
                                        <SheetDescription>Configure external services for your MultiAgents.</SheetDescription>
                                    </SheetHeader>
                                    <div className="mt-4 text-sm text-muted-foreground">
                                        {Api.length === 0 ? (
                                            ...
                                            <ul className="space-y-2">
                                                {Api.map((s: any) => (
                                                    <li key={s.provider} className="flex items-center gap-2 p-2 rounded-lg border">
                                ...
                                <SheetContent side="right" className="w-full sm:w-[500px]">
                                    <SheetHeader>
                                        <SheetTitle>
                                            <div className="flex items-center gap-2">
                                                Chat History
                                                <Button onClick={onResetHistory} disabled={store.history.length === 0 || resetMsgMutation.isPending} size="sm" variant="destructive" className="h-6 text-xs px-2">
