      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {nodes.map((element, idx) => {
          ...
          return (
            <Card
              key={idx}
              className={`relative transition-all duration-300 p-4 ${isActive ? "border-cyan-500 shadow-[0_0_20px_-5px_rgba(6,182,212,0.4)] bg-cyan-50/5 dark:bg-cyan-950/10 z-10" : "border-border opacity-80 hover:border-cyan-500/50"}`}
            >
              {isActive && (
              ...
              <div className="flex justify-between items-start">
                <div className="flex flex-col gap-2">
                  <div className="flex items-center gap-2 text-lg max-lg:text-sm font-semibold">
                    <Bot
                      className={`text-cyan-500 dark:text-white ${isActive ? "animate-bounce" : ""}`}
                    />
                    {element.name}
                    <p
                      className={`text-[10px] px-2 py-1 rounded-full border transition-all duration-300 ${isActive ? "bg-cyan-500 text-white border-cyan-400 shadow-[0_0_8px_rgba(6,182,212,0.5)]" : "bg-cyan-500/10 text-cyan-500 border-cyan-500/20"}`}
                    >
                  ...
                  <div className="text-xs flex gap-1 items-center">
                    <img
                      src={BRAND_ASSETS[element.provider.toLowerCase()]}
                      className="bg-white rounded-lg p-0.5 w-5 h-5"
                    />
                  ...
                  <div className="text-xs">
                    ...
                    <span className="text-green-500">
                      {" "}
                      {ToolLabels[element.tool] || element.tool}{" "}
                    </span>
                  </div>
                </div>
                <DropdownMenu>
              ...
              <div
                className="mt-3 flex-1 overflow-y-auto max-h-75 text-sm"
                ...
                <div className="whitespace-pre-wrap">
                  {element.output ? (
                    ...
                    <motion.span
                      ...
                      style={{
                        backgroundImage:
                          "linear-gradient(90deg, #6b7280 0%, #f3f4f6 50%, #6b7280 100%)",
