...
export const GithubMessageList = () => {
  ...
  if (store.loadingfetch || loadinggithub) {
    return (
      <div
      ...
      <div className="mx-auto max-w-5xl py-5">
        ...
        {hasMessages ? (
                          ...
                          .map((tool: any) => (
                            <Collapsible key={tool.id} className="w-full space-y-2">
                              <CollapsibleTrigger asChild>
                                <Button
                                  variant="ghost"
                                  className={`group flex items-center gap-2 px-3 py-1 rounded-full text-xs font-medium transition-all active:scale-95 ${tool.status === "done" ? "text-muted-foreground shadow-sm" : "text-muted-foreground"}`}
                                >
                                  {tool.status === "loading" && (
                                  ...
                                  {tool.status === "loading" ? (
                                    ...
                                    >
                                      {Githubtool[tool.name.toLowerCase()]}
                                    </motion.span>
                                  ) : (
                                    <span>{Githubtool[tool.name.toLowerCase()]}</span>
                                  )}
                                  {tool.status !== "loading" && (
                                    <ChevronDown
                                      size={15}
          ...
          <div className="min-h-[50vh] flex flex-col gap-2 justify-center items-center">
            ...
            <p className="text-sm text-muted-foreground">
              Send Message To Get Started Github Agenting.
            </p>
          </div>
        )}
