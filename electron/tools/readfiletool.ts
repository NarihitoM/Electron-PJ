...
export const readfiletool = tool(
    async ({ filepath, encoding }) => {
        try {
            ...
            const truncated = totalLines > 500 ? lines.slice(0, 500).join("\n") + `\n\n... (${totalLines - 500} more lines omitted)` : content as string;
            return truncated;
        } catch (err: any) {
    ...
    {
        name: "read_file",
        description: "Read the contents of a text file from the project directory. Supports utf-8, latin1, base64, hex, and binary encodings. Truncates output to 500 lines.",
        schema: z.object({
            filepath: z.string().describe("Relative or absolute path to the file within the project directory"),
            encoding: z.enum(["utf-8", "latin1", "base64", "hex", "binary"]).optional().describe("File encoding (default: utf-8)")
        })
    }
);
