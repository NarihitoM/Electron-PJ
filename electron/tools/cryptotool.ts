...
export const cryptotool = tool(
    async ({ action, text, algorithm, length, use_uppercase, use_lowercase, use_digits, use_symbols }) => {
        try {
            switch (action) {
                case "hash": {
                ...
                case "generate_password": {
                    ...
                    if (use_symbols !== false && !/[!@#$%^&*()_+\-=\[\]{}|;:,.<>?]/.test(password)) checks.push("symbol");
    ...
    {
        name: "crypto_tool",
        description: "Generate passwords, compute hashes (MD5, SHA1, SHA256, SHA384, SHA512), generate UUIDs, and encode/decode Base64.",
        schema: z.object({
            action: z.enum(["hash", "generate_password", "uuid", "base64_encode", "base64_decode"]).describe("Operation to perform"),
            text: z.string().optional().describe("Input text for hash, base64_encode, or base64_decode actions"),
            algorithm: z.string().optional().describe("Hash algorithm: md5, sha1, sha256, sha384, sha512 (default: sha256)"),
            length: z.number().optional().describe("Password length (default: 16, max: 128)"),
            use_uppercase: z.boolean().optional().describe("Include uppercase letters in password (default: true)"),
            use_lowercase: z.boolean().optional().describe("Include lowercase letters in password (default: true)"),
