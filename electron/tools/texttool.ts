...
export const texttool = tool(
  ...
  },
  {
    name: "text_tool",
    description:
      "Process and analyze text. Actions: strip_html (remove HTML tags), truncate (cut to length), split (divide by separator), stats (word/char/sentence/paragraph/token counts), count_tokens (estimate LLM tokens).",
    schema: z.object({
      action: z
        .enum(["strip_html", "truncate", "split", "stats", "count_tokens"])
        .describe("Operation to perform on the text"),
      text: z.string().optional().describe("Input text to process"),
      max_length: z
        .number()
        .optional()
        .describe("Maximum length for truncate action (default: 1000)"),
      separator: z
        .string()
        .optional()
        .describe("Separator for split action (default: double newline)"),
    }),
  },
);
