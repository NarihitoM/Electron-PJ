...
export const emailtool = tool(
  ...
  },
  {
    name: "send_email",
    description:
      "Send an email. Priority: 1) custom SMTP host if provided, 2) backend API (uses server's Gmail OAuth or user's stored SMTP creds), 3) SMTP from env vars. Supports plain text and HTML.",
    schema: z.object({
      ...
      smtp_host: z
        .string()
        .optional()
        .describe("SMTP server hostname (uses env defaults if omitted)"),
