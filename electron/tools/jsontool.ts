...
export const jsontool = tool(
    ...
    {
        name: "json_tool",
        description: "Process JSON data. Actions: format (pretty-print), validate (check syntax), query (extract by dot-path like 'data.items[0].name'), merge (merge two objects), keys (list top-level keys).",
        schema: z.object({
            action: z.enum(["format", "validate", "query", "merge", "keys"]).describe("Operation to perform on the JSON data"),
            json: z.string().describe("The JSON string to process").optional(),
            path: z.string().optional().describe("Dot-notation path for query action, e.g. 'data.items[0].name'"),
            indent: z.number().optional().describe("Indentation spaces for format/merge (default: 2)"),

            merge_with: z.string().optional().describe("Second JSON string to merge with the first (for merge action)")
        })
    }
);
