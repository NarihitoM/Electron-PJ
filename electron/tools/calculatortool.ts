...
function parseAtom(tokens: string[], pos: { i: number }): number {
    ...
    if (SAFE_MATH_FUNCS[token] !== undefined) {
        pos.i++;
        if (pos.i >= tokens.length || tokens[pos.i] !== "(") throw new Error(`Expected ( after ${token}`);
        pos.i++;
        const args: number[] = [];
        if (pos.i < tokens.length && tokens[pos.i] !== ")") {
...
export const calculatortool = tool(
    ...
    {
        name: "calculate",
        description: "Evaluate a mathematical expression. Supports all Math methods (pow, sqrt, sin, cos, tan, log, abs, round, floor, ceil, min, max), constants (pi, e), operators (+, -, *, /, ^), and parentheses. Use Math.pow(x,y) or x^y for exponents.",
        schema: z.object({
            expression: z.string().describe("The mathematical expression to evaluate, e.g. 'Math.pow(2, 10)', 'pi * 5 + 3', 'Math.sqrt(144)'")
        })
    }
);
